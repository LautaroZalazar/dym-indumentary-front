export const validationPassword = (validatePassword: string) => {
	const validations = [
		...
		{
			condition: /[a-z]/.test(validatePassword),
		...
		{
			condition: /[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?]/.test(validatePassword),
...
export const validationSamePassword = (
	validationPassword: string,
	validationSamePassword: string
) => {
	const validations = [
		{
			condition: validationSamePassword !== validationPassword,
...
export const validationEmail = (email: string) => {
	const validations = [
		...
		{
			condition: /[a-zA-Z0-9.-]+\.[a-zA-Z]{2,4}$/.test(email),
			message: 'Luego del punto, mminimo 2 caracteres y maximo 4',
		},
	];
	return validations;
};
